 ...
 * @ngdoc overview
 * @name gloocodeApp
 * @description Code Challenge for Gloo Developer Position
 * # gloocodeApp
 *
...
.controller("userController", function($scope,$http){
        ...
        $http.post("action.php", data, config).success(function(response){
            if(response.status == 'OK'){
                if(type == 'edit'){
                    ...
                    $scope.users[$scope.index].notes = $scope.tempUserData.notes;
    ...
    $scope.deleteUser = function(user){
        var conf = confirm('Are you sure to delete the resource?');
        if(conf === true){
            var data = $.param({
                'id': user.id,
                'type':'delete'
